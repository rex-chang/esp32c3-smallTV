...
String cityCode = "101280601";  //天气城市代码，北京’: ‘101010100’,‘上海’: ‘101020100’,‘重庆’: ‘101040100’,‘深圳’: ‘101280601’,
//----------------------------------------------------

//LCD屏幕相关设置
TFT_eSPI tft = TFT_eSPI();  // 引脚请自行配置tft_espi库中的 User_Setup.h文件
TFT_eSprite clk = TFT_eSprite(&tft);
#define LCD_BL_PIN 1    //LCD背光引脚
...
void SmartConfig(void)
{
  ...
  tft.pushImage(0, 0, 240, 240, wd);
  //tft.pushImage(0, 0, 240, 240, qr); //二维码
  ...
  if(Serial.available()>0)
  {
    ...
    if(SMOD=="0x01")//设置1亮度设置
    {
      ...
      if(LCDBL>=0 && LCDBL<=100)
      {
        ...
        SMOD = "";
        Serial.printf("亮度调整为：");
		analogWrite(LCD_BL_PIN, 255-LCD_BL_PWM);
    ...
    else
    {
      ...
      {
        Serial.println("WiFi reset succ!!");
        SMOD = "";
        ESP.restart();
      }
      else if(SMOD=="0x07")
...
void handleconfig()
{
      ...
      server.hasArg("web_upwe_t") || server.hasArg("web_DHT11_en") || server.hasArg("web_set_rotation") || server.hasArg("web_disp_mode"))
  {
    ...
    {
      EEPROM.write(UpWeT_addr, web_upt);//天气更新时间
      EEPROM.commit();//保存更改的数据
      delay(5);
  ...
  String content = "<html><style>html,body{ background: #1aceff; color: #fff; font-size: 10px;}</style>";
        ...
        content += "City Code:<br><input type='text' name='web_ccode' placeholder='101280601'><br>";
        content += "<br>Back Light(1-100):(default:50)<br><input type='text' name='web_bl' placeholder='50'><br>";
...
void Web_sever_Win()
{
  ...
  String ip = "IP:" + IP_adr.toString();
  String wifi = "Connect to WiFi:" + WiFi.SSID();

  clk.setColorDepth(8);
  clk.createSprite(240, 70);//创建窗口
  clk.fillSprite(0x0000);   //填充率

  // clk.drawRoundRect(0,0,200,100,5,0xFFFF);       //空心圆角矩形
  clk.setTextDatum(ML_DATUM);   //设置文本数据
  clk.setTextColor(TFT_GREEN, 0x0000);
  clk.drawString(wifi,10,5,2);
  clk.setTextColor(TFT_WHITE, 0x0000);
//  clk.drawString("http://sd3.local",100,35,4);
  clk.drawString(ip,10,35,4);
//  clk.setTextColor(TFT_GREEN, 0x0000);
//  clk.drawString("IP:",45,55,2);
//  clk.drawString(IP_adr.toString(),110,55,2);
...
void Webconfig()
{
  ...
  WiFiManagerParameter  custom_bl("LCDBL","LCD BackLight(1-100)","50",3);
  #if DHT_EN
  ...
  WiFiManagerParameter  custom_cc("CityCode","CityCode","101280601",9);
...
//  wm.setConfigPortalTimeout(30); // auto close configportal after n seconds
  ...
  // res = wm.autoConnect(); // auto generated AP name from chipid
   wm.setAPStaticIPConfig(IPAddress(192,168,4,1), IPAddress(192,168,4,1), IPAddress(255,255,255,0)); // set static ip,gw,sn
...
void setup()
{
  ...
  Udp.begin(localPort);
  Serial.println("等待同步...");
  setSyncProvider(getNtpTime);//同步NTP时间
  setSyncInterval(300);//以秒为单位，5分钟更新一次
